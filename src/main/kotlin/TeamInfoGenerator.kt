...
class TeamInfoGenerator {
    ...
    private val outputDir = Path.of("generated").createDirectories().toString()
    ...
    fun generateMainSVG(data: Data) {
        val svgText: String = if (data.university.fullName.length <= 45) {
            ...
            File("src/main/resources/svgBase/main2.svg").readText(Charsets.UTF_8)
                ...
                .replace(
                    "{FullUniversityName_2}",
                    data.university.fullName.substring(45, data.university.fullName.length)
                )
        }
        ...
        File(outputDir + "/" + data.id.toString() + "_main.svg").writeText(replaced, Charsets.UTF_8)
    }
